...
UserSchema.statics.isEmailDB = async (req, res, next) => {
  ...
  if (result) {
    bcryptjs.hash(toString(result.email), 10, (err, hash) => {
      if (err) {
        return next(err);
      } else {
        token = hash;
...
UserSchema.statics.changePassword = async (req, res, next) => {
  try {
    ...
    let match = await bcryptjs.compare(req.body.oldPassword, user.password);
    if (match) {
      if (req.body.newPassword === req.body.newPasswordConf) {
        await bcryptjs.hash(req.body.newPassword, 10, async (err, hash) => {
          if (err) {
            return next(err);
          } else {
            await User.findOneAndUpdate(
