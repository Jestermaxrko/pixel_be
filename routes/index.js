...
const confirmEmail = user => {
  ...
  bcryptjs.hash(user.email, 10, (err, hash) => {
    if (err) {
      return next(err);
    }
...
router.post('/register', User.validate, async (req, res, next) => {
  try {
    const isInDB = await User.isUserInDB(req.body.email, req.body.nickname);
...
router.get("/verify", async (req, res, next) => {
  HashForEmail.findOne({ hash: `${req.query.hash}` }, async (err, hashForEmail) => {
    try {
      let newUser = {
